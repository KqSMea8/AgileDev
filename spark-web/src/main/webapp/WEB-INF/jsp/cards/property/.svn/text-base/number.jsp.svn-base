...
<%@ taglib prefix="spark" uri="http://spark.baidu.com/tags"%>
	<c:choose>
		...
		<c:when test="${_showType == 'edit'}">
			<input type="text" id="property_${_value.cardProperty.id}" name="property_${_value.cardProperty.id}" value='<c:out value="${_value.displayValue}"/>' onblur="trigBlur${_value.cardProperty.id}();"/>
			<label style="color:red"></label>
			<script type="text/javascript">
				function trigBlur${_value.cardProperty.id}(){
					var me = $("#property_${_value.cardProperty.id}");
					var patrn = /^-?\d+$/; 
					if (me.val() && !patrn.exec(me.val())){
						foc(me);
						me.next().text("*");
					}else{
						me.next().text("");
