...
<spark:template page="../templates/manage.jsp" menu="group">
	<c:if test="${group.id == null}"><%--卡片id为空，创建新卡片 --%>
		...
		<c:url var="actionPath" value="/spaces/${space.prefixCode}/groups"></c:url>
	</c:if>
	<c:if test="${group.id != null}">
		<c:set var="method" value="put"/>
		<c:url var="actionPath" value="/spaces/${space.prefixCode}/groups/${group.id}"></c:url>
	</c:if >
	<c:url var="listUrl" value="/spaces/${space.prefixCode}/groups/list"/>
	<div class="title-bar">
		<h3>
	...
	<form:form action="${ actionPath }" method="${method}" modelAttribute="group">
		...
		<table cellspacing="0" class="details-table">
			<tbody>
				<tr>
					...
					<td><c:if test="${group.owner.id==space.id||group.id == null}">
						<form:input path="name"/>
						</c:if>
						<c:if test="${group.id != null&&group.owner.id!=space.id}">
		...
		<button type="button" onclick="location.href = '${ listUrl }'"><fmt:message key="button.back"/></button>
	</form:form>
</spark:template>